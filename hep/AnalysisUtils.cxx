...
bool AnalysisUtils::ExtractSignalRooFit(TH1 * sigBg, TH1 * bg, json & cfg, TList * output, TH1 * results)
{
  ...
  if (!fitType.CompareTo("rootfit")) {
    ...
    fitResult = model.fitTo(data, RooFit::Range(minFit, maxFit), RooFit::PrintLevel(-1));
    // if (!fitResult) {
    //   accepted = false;
    ...
    model.plotOn(frame, RooFit::Components("pol"), RooFit::LineStyle(kDashed), RooFit::LineColor(kBlue));
