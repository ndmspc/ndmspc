...
/*********************** Constructor ***********************/
NLogExporter::NLogExporter(std::ostream & sout, opentelemetry::logs::Severity min_severity) noexcept
    : sout_(sout), min_severity_(min_severity)
{
}

/*********************** Exporter methods ***********************/

std::unique_ptr<sdklogs::Recordable> NLogExporter::MakeRecordable() noexcept
{
  return std::unique_ptr<sdklogs::Recordable>(new sdklogs::ReadWriteLogRecord());
}

sdkcommon::ExportResult NLogExporter::Export(
    const opentelemetry::nostd::span<std::unique_ptr<sdklogs::Recordable>> & records) noexcept
{
  ...
  for (auto & record : records) {
    ...
    if (log_record->GetSeverity() >= min_severity_) {
      ...
      auto time_since_epoch =
          std::chrono::duration_cast<std::chrono::nanoseconds>(observed_timestamp.time_since_epoch());
...
bool NLogExporter::ForceFlush(std::chrono::microseconds /* timeout */) noexcept
{
  sout_.flush();
  return true;
}

bool NLogExporter::Shutdown(std::chrono::microseconds) noexcept
{
  is_shutdown_ = true;
  return true;
}

bool NLogExporter::isShutdown() const noexcept
{
  return is_shutdown_;
}
