...
/// \cond CLASSIMP
ClassImp(Ndmspc::NCloudEvent);
/// \endcond

namespace Ndmspc {

NCloudEvent::NCloudEvent(std::string id, std::string source, std::string specVersion, std::string type)
    : TObject(), fId(id), fSource(source), fSpecVersion(specVersion), fType(type)
{
}
NCloudEvent::NCloudEvent(THttpCallArg * arg)
{
  HandleNCloudEventRequest(arg);
}
NCloudEvent::~NCloudEvent() {}

void NCloudEvent::HandleNCloudEventRequest(THttpCallArg * arg)
{
  Clear();
...
std::string NCloudEvent::GetInfo() const
{

  return TString::Format("NCloudEvent: id=%s source=%s specVersion=%s type=%s datacontenttype=%s data=%s", fId.c_str(),
