...
Bool_t HnSparseStress::Generate(THnSparse * h, Long64_t size, Long64_t start)
{
    ...
    fNFilledMax = size;
    if (fDebugLevel > 0)
        Printf("dimensions=%d chunkSize=%d nFillMax=%lld start=%lld", h->GetNdimensions(), h->GetChunkSize(),
               fNFilledMax, start);
    ...
    if (start > 0) {
        ...
        for (Int_t i = 0; i < h->GetNdimensions(); ++i) {
            ...
            if (allBins > kMaxLong64 / h->GetAxis(i)->GetNbins()) {
                Printf("Error: Product of all bins is higer then %lld !!! Do not use --start in this case !!!",
                       kMaxLong64);
                return kFALSE;
            }
        ...
        for (Int_t i = h->GetNdimensions() - 1; i >= 0; --i) {
            ...
            cStart[i] = bx;
            if (fDebugLevel > 0)
                Printf("i=%d x=%d startIndex=%lld allBins=%lld cStart[%d]=%d", i, bx, startIndex, allBins, i,
                       cStart[i]);
        }
