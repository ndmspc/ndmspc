...
int main(int argc, char ** argv)
{
  ...
  run->add_option("-f,--file", runInputFile, "Header file generated by 'import' command containing compiled functions.")
    ...
    run->add_option("-m,--methods", methods, "Integration methods (comma-separated: vegas,suave,divonne,cuhre). Use 'method:config.json' to specify per-method JSON configuration files. Examples: 'vegas', 'vegas,suave', 'vegas:vegas_config.json,suave:suave_config.json'")
      ->default_val("vegas");
    run->add_option("--method", methodList, "Integration method (repeatable, e.g. --method vegas:foo.json --method suave:bar.json)")
      ->expected(-1);
  ...
  auto run_fun = [&]() {
    try {
      std::vector<std::string> rerunPairs;
      ...
      NLogInfo("Importing functions from: %s", runInputFile.c_str());
      ...
      manager.SetFunctions(*functions, *labels, dims);
      ...
      // Parse --method (repeatable)
      for (const auto& entry : methodList) {
        std::string methodEntry = entry;
        methodEntry.erase(methodEntry.begin(), std::find_if(methodEntry.begin(), methodEntry.end(), [](int ch) {
          return !std::isspace(ch);
        }));
        methodEntry.erase(std::find_if(methodEntry.rbegin(), methodEntry.rend(), [](int ch) {
      ...
      std::string jsonFileName = runInputFile;
      size_t hPos = jsonFileName.rfind(".h");
      if (hPos != std::string::npos) {
        jsonFileName = jsonFileName.substr(0, hPos);
      }
      jsonFileName += "_vegas.json";
      std::ofstream jsonFile(jsonFileName);
      if (jsonFile.is_open()) {
        jsonFile << opts.to_json().dump(4) << std::endl;
        jsonFile.close();
        NLogInfo("Saved current Vegas options to: %s", jsonFileName.c_str());
        rerunPairs.push_back("vegas:" + jsonFileName);
      } else {
        NLogError("Could not write options JSON file: %s", jsonFileName.c_str());
      }
      ...
      jsonFileName += "_suave.json";
      std::ofstream jsonFile(jsonFileName);
      if (jsonFile.is_open()) {
        jsonFile << opts.to_json().dump(4) << std::endl;
        jsonFile.close();
        NLogInfo("Saved current Suave options to: %s", jsonFileName.c_str());
        rerunPairs.push_back("suave:" + jsonFileName);
      } else {
        NLogError("Could not write options JSON file: %s", jsonFileName.c_str());
      }
      ...
      jsonFileName += "_divonne.json";
      std::ofstream jsonFile(jsonFileName);
      if (jsonFile.is_open()) {
        jsonFile << opts.to_json().dump(4) << std::endl;
        jsonFile.close();
        NLogInfo("Saved current Divonne options to: %s", jsonFileName.c_str());
        rerunPairs.push_back("divonne:" + jsonFileName);
      } else {
        NLogError("Could not write options JSON file: %s", jsonFileName.c_str());
      }
            ...
            result = manager.RunCuhre(opts);
                ...
                jsonFileName += "_cuhre.json";
                std::ofstream jsonFile(jsonFileName);
                if (jsonFile.is_open()) {
                  jsonFile << opts.to_json().dump(4) << std::endl;
                  jsonFile.close();
                  NLogInfo("Saved current Cuhre options to: %s", jsonFileName.c_str());
                  rerunPairs.push_back("cuhre:" + jsonFileName);
                } else {
                  NLogError("Could not write options JSON file: %s", jsonFileName.c_str());
                }
          }
          ...
          result.Print();
      // Print all rerun messages at the end
      if (!rerunPairs.empty()) {
        std::string joined;
        for (const auto& pair : rerunPairs) {
          joined += " --method " + pair;
        }
        NLogInfo("To rerun with this configuration:%s", joined.c_str());
      }
